...
if [[ -z "$resolveip" && -z "$resolvedns" ]]; then
    PS3='Escolha sua opção preferida, IP ou DNS/Domínio:'
    WAN=("IP" "DNS/Domain")
...
while ! [[ $CHECK_RUSTDESK_READY ]]; do
  ...
  echo -ne "Rustdesk Relay ainda não está pronto...${NC}\n"
  sleep 3
done
...
function setuphttp () {
    ...
    echo "Instalando o  Go HTTP Server"
    if [ "${ARCH}" = "x86_64" ] ; then
...
EOF
    ...
    echo -e "Seu endereço de IP/DNS ${wanip}"
    echo -e "Sua Chave é ${key}"
    echo -e "Agora instale o Rustdesk Client e adicione sua chave e o seu IP/DNS nas configurações de rede do rustdesk client."
    echo -e "Você pode acessar seus scripts de instalação para clientes acessando http://${wanip}:8000"
    echo -e "O nome de usuário é admin e a senha é ${admintoken}"
    if [[ -z "$http" ]]; then
        echo "Pressione qualquer tecla pra finalizar"
        while [ true ] ; do
        read -t 3 -n 1
        if [ $? = 0 ] ; then
        exit ;
        else
        echo "Pressiona qualquer tecla"
        fi
        done
        break
    fi
...
if [[ -z "$http" ]]; then
    ...
    echo "Pronto, finalizad! Pressione qualquer tecla pra finalizar a instalação"
    while [ true ] ; do
    read -t 3 -n 1
    if [ $? = 0 ] ; then
    exit ;
    else
    echo "Pressiona alguma tecla pra terminar!"
    fi
    done
    break
    ;;
